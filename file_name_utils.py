...
def validate_and_return_input_file_name(raw_input_result_file_name):
    print(f"validate_and_return_input_file_name start: {raw_input_result_file_name}")
    sanitized_user_input = sanitize_filename(raw_input_result_file_name).strip()
    files_with_same_name = glob.glob(f"resources/lang_percentage_result/{sanitized_user_input}*.p")
...
def get_next_number(file_numbers):
    file_numbers_as_ints = [int(n) if n else 0 for n in file_numbers]
...
def extract_number(absolute_file_name, fn_without_extension):
    ...
    step1 = re.sub(f"^resources/lang_percentage_result/{fn_without_extension}", '', absolute_file_name)
